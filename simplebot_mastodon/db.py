...
class DBManager:
    ...
    def remove_account(self, id: int) -> None:
        with self.db:
            self.db.execute("DELETE FROM pchats WHERE account=?", (id,))
    ...
    def get_account_by_id(self, id: int) -> Optional[sqlite3.Row]:
        return self.db.execute("SELECT * FROM accounts WHERE id=?", (id,)).fetchone()
    ...
    def add_pchat(self, gid: int, contact: str, id: int) -> None:
        with self.db:
            self.db.execute(
                "INSERT INTO pchats VALUES (?,?,?)", (gid, contact.lower(), id)
            )
    ...
    def get_pchats(self, id: int) -> List[sqlite3.Row]:
        return self.db.execute("SELECT * FROM pchats WHERE account=?", (id,)).fetchall()

    def get_pchat_by_contact(self, id: int, contact: str) -> Optional[sqlite3.Row]:
